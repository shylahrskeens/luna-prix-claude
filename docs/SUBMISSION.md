...
is the foundation a competitive ladder needs. The server and its client exist
and have been run end to end as two headless Node clients on localhost. **No
server is hosted, and single-player results are built locally** — the
authoritative path is proven, not in service. That is a foundation, not a
roadmap.
...
**Material AI use.** The code was written with Claude (Claude Code) across
several sessions, from a prompt bible supplied by the owner, who directed the
design and made every product decision. Claude wrote the implementation and the
validators and harnesses that found the defects — several of which were
invisible from playing and only showed up in a terminal. Some earlier
exploratory work was done in Grok Build; none of that code is in this
repository.

**Pre-existing work and starters.** None. This repository was started from an
empty directory for the Vibeathon. There is no template, boilerplate or
starter kit in it.

**Dependencies.** One runtime dependency: **three.js** (MIT). Build and test
only: TypeScript, Vite, esbuild, `ws`. No framework, no game engine, no
physics library.

**Assets.** No third-party assets ship in the build. Every model, track,
effect and sound is generated by code in this repository. The UI typeface is
Inter, loaded from Google Fonts (SIL Open Font Licence) — the only request the
page makes off its own origin. The official Axie 3D asset pack was downloaded
and evaluated during development; it is **not** used, not shipped, and is
excluded from the repository.

**Contributors.** One person: the repository owner. No team, no outside
contributors.
...
In `docs/evidence/screens/`: the gator pit on approach and mid-leap, the Mega
Ramp result, the garage, and the Cloudforge hangar. Five shots, listed in that
folder's README.
...
**Where it runs.** Any browser with WebGL2. It needs no install, no wallet and
no backend, and quality steps itself down once if the frame time will not hold.
It is also a installable web app, so the same build runs as a windowed app on
every platform below and starts without a network once it has been opened.

| Platform | Link | How to run it |
|---|---|---|
| **Browser** | https://shylahrskeens.github.io/luna-prix-claude/ | Open the link. Nothing to install. |
| **Windows** | same link | Open in Chrome or Edge, then **Install Luna Prix** from the address bar (or ⋮ → Cast, save and share → Install page as app). It opens in its own window. |
| **macOS** | same link | Open in Chrome or Edge, then **Install Luna Prix** from the address bar. Safari: File → Add to Dock. |
...
There is no separate native binary: one build, installed through the browser.
...
- Review commit: the commit of the most recent successful **Deploy to GitHub
  Pages** run, which is the commit that produced the link above. Read it with
  `gh run list --workflow=pages.yml --json headSha` or from the Actions tab.
  The playable link is always built from that commit by CI, never uploaded by
  hand.
